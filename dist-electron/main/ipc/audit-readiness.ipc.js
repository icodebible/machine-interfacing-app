...
function registerAuditReadinessIpc() {
     ...
     */
    electron_1.ipcMain.handle(channels_1.IPC_CHANNELS.AUDIT_EVENTS_QUERY, (_event, query) => audit.query(query ?? {}));
    electron_1.ipcMain.handle(channels_1.IPC_CHANNELS.AUDIT_EVENTS_SUMMARY, (_event, days) => audit.summary(days ?? 7));
    electron_1.ipcMain.handle(channels_1.IPC_CHANNELS.DEPLOYMENT_READINESS_CHECK, () => readiness.run());
}
