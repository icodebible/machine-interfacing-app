...
exports.machineHostLogService = exports.MachineHostLogService = void 0;
...
class MachineHostLogService {
    activeBySession = new Map();
    activeByMachine = new Map();
    rawActiveBySession = new Map();
    rawActiveByMachine = new Map();
    writeChains = new Map();
     ...
     */
    getRootDirectory() {
        return path_1.default.join(electron_1.app.getPath('userData'), 'machine-logs');
    }
    getMachineDirectory(machineId) {
        return path_1.default.join(this.getRootDirectory(), `machine-${safeSegment(machineId, 'unknown')}`);
    }
    /**
     ...
     */
    getRawRootDirectory() {
        return path_1.default.join(electron_1.app.getPath('userData'), 'machine-raw-logs');
    }
    getRawMachineDirectory(machineId) {
        return path_1.default.join(this.getRawRootDirectory(), `machine-${safeSegment(machineId, 'unknown')}`);
    }
    startSession(input) {
    ...
    endSession(input) {
        ...
        const filePath = target?.filePath ?? (input.startedAt && input.mode
    ...
    appendRawMachineMessage(input) {
        ...
        if (!active || String(active.mode).toUpperCase() !== 'LIVE')
            return null;
        const timestamp = input.timestamp || nowIso();
        const lines = [
    ...
    async getInfo(machineId) {
        ...
        await Promise.all([
            fs_1.promises.mkdir(machineDirectory, { recursive: true, mode: 0o700 }),
            fs_1.promises.mkdir(rawMachineDirectory, { recursive: true, mode: 0o700 }),
        ]);
    ...
    async openMachineDirectory(machineId) {
        const info = await this.getInfo(machineId);
        const error = await electron_1.shell.openPath(info.machineDirectory);
        if (error)
            throw new Error(error);
        return true;
    }
    async openRawMachineDirectory(machineId) {
        const info = await this.getInfo(machineId);
        const error = await electron_1.shell.openPath(info.rawMachineDirectory);
        if (error)
            throw new Error(error);
        return true;
    }
    ...
    sessionFilePath(input) {
        ...
        return path_1.default.join(this.getMachineDirectory(input.machineId), fileName);
    }
    rawSessionFilePath(input) {
        const machineName = safeSegment(input.machineName, 'machine');
        const fileName = `${compactTimestamp(input.startedAt)}--LIVE--${machineName}--${safeSegment(input.sessionId, 'session')}--RAW.txt`;
        return path_1.default.join(this.getRawMachineDirectory(input.machineId), fileName);
    }
    fallbackFile(machineId, timestamp = nowIso()) {
        const day = timestamp.slice(0, 10);
        return path_1.default.join(this.getMachineDirectory(machineId), `${day}--machine-events.txt`);
    }
    async listRecentTextFiles(directory, label) {
        try {
            return (await fs_1.promises.readdir(directory, { withFileTypes: true }))
            ...
            logger_1.logger.warn(`Unable to enumerate ${label} log files`, error);
            return [];
        }
    }
    enqueue(filePath, content) {
        ...
        const current = previous
            .catch(() => undefined)
            .then(async () => {
            await fs_1.promises.mkdir(path_1.default.dirname(filePath), { recursive: true, mode: 0o700 });
            await fs_1.promises.appendFile(filePath, content, { encoding: 'utf8', mode: 0o600 });
        })
            .catch((error) => {
            logger_1.logger.error(`Failed to append machine host log: ${filePath}`, error);
        });
...
exports.MachineHostLogService = MachineHostLogService;
exports.machineHostLogService = new MachineHostLogService();
