...
class ResultFlowService {
    ...
    routing = new routing_rule_service_1.RoutingRuleService();
    ...
    queueApprovedResult(normalizedResultId, policyId) {
        ...
        `)
            .get(normalizedResultId);
        const routeResolution = this.resolveRoutableTargetIds(normalizedResultId, policyId, this.approvals.listPolicyTargetIds(policyId));
        const targetIds = routeResolution.targetIds;
        const queueResult = this.queueTargets(normalizedResultId, targetIds);
        queueResult.warnings.push(...routeResolution.warnings);
        const finalStatus = queueResult.queuedCount > 0 ? 'QUEUED' : 'APPROVED';
        ...
        for (const target of targets) {
            try {
                const existing = db
                    ...
                    .get(normalizedResultId, target.id);
                if (existing?.id) {
                    result.queuedCount += 1;
                    result.existingQueuedCount += 1;
                    continue;
                ...
                result.errors.push(`${target.name || target.id}: ${error?.message ?? 'Unable to create outbound queue item.'}`);
            }
        }
        return result;
    }
    resolveRoutableTargetIds(normalizedResultId, policyId, configuredTargetIds) {
        const db = (0, db_1.getDb)();
        const warnings = [];
        ...
        if (!seededTargets.length && policyId) {
            for (const id of this.approvals.listPolicyTargetIds(policyId)) {
                if (id && !seededTargets.includes(id))
                    seededTargets.push(id);
            }
        }
        ...
        if (!seededTargets.length) {
            const lisTargets = db
                .prepare(`SELECT id, name FROM targets WHERE enabled = 1 AND UPPER(type) IN ('LIS', 'OPENMRS', 'OPENMRS_LIS') ORDER BY name ASC, id ASC`)
                .all();
            if (lisTargets.length === 1) {
                return {
                    targetIds: [lisTargets[0].id],
            ...
            if (lisTargets.length > 1) {
                warnings.push(`No routing rule or policy target matched and multiple enabled LIS/OpenMRS targets exist. Configure Routing Rules before queueing.`);
            }
        }
        if (!seededTargets.length) {
            const enabledTargets = db
            ...
            if (!warnings.length) {
                warnings.push(enabledTargets.length
    ...
    buildPostApprovalMessage(queueResult, targetIds) {
        const warnings = queueResult.warnings ?? [];
        if (queueResult.queuedCount > 0 && queueResult.errors.length === 0) {
            return warnings.length ? warnings.join(' | ') : null;
        }
