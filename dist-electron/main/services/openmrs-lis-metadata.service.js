...
exports.OpenMrsLisMetadataService = void 0;
// // type TargetRow = {
// //     id: string;
// //     type: string;
// //     name: string;
...
class OpenMrsLisMetadataService {
    ...
    async discover(input) {
        ...
        const sampleUuid = String(input.sampleUuid ?? '').trim() ||
            ...
            null;
        const sampleLabel = lookup?.sample?.label || lookup?.label || String(input.sampleId ?? '').trim() || null;
        const allocations = await this.fetchOptional(target, sampleUuid ? `/lab/allocationsbysample?uuid=${encodeURIComponent(sampleUuid)}` : '', headers, allowInsecureTls, timeout, !!sampleUuid, warnings, 'sample allocations');
        const settings = {};
        for (const property of SETTINGS) {
            const payload = await this.fetchOptional(target, `/systemsetting?q=${encodeURIComponent(property)}&v=full`, headers, allowInsecureTls, timeout, true, warnings, property);
            settings[property] = payload?.results?.[0]?.value ?? null;
        }
    ...
    async fetchOptional(target, path, headers, allowInsecureTls, timeout, shouldFetch, warnings, label) {
        if (!shouldFetch || !path)
            return null;
        const url = this.restUrl(target, path);
        const res = await this.http.getJson(url, headers, allowInsecureTls, timeout);
    ...
    async fetchOpenMrsUsers(target, query, headers, allowInsecureTls, timeout, warnings) {
        ...
        let pageCount = 0;
        while (results.length < maxUsers && pageCount < 250) {
            const path = `/user?startIndex=${startIndex}` +
                `&limit=${limit}` +
                `&v=${encodeURIComponent(userView)}` +
                `&q=${encodeURIComponent(query)}`;
            const page = await this.fetchOptional(target, path, headers, allowInsecureTls, timeout, true, warnings, `OpenMRS users page starting at ${startIndex}`);
            if (!page)
                break;
            const rows = Array.isArray(page?.results) ? page.results : [];
            const pageLinks = Array.isArray(page?.links) ? page.links : [];
    ...
    toUserList(payload) {
        ...
        const seen = new Set();
        return rows
            .filter((row) => row?.uuid && !this.isRetired(row))
            .map((row) => ({
            uuid: String(row.uuid),
            username: row.username ? String(row.username) : null,
            display: String(row.person?.display ?? row.display ?? row.username ?? row.uuid),
    ...
    extractParameters(payload) {
        ...
        const visit = (node, parent = null) => {
            ...
            if (typeof node !== 'object')
                return;
            const concept = this.firstObject(node.concept, node.testParameter, node.parameter, node.parameterConcept, node.testParameterConcept, node.test?.concept, node.order?.concept);
    ...
    async attachConceptDetails(target, parameters, headers, allowInsecureTls, timeout, warnings) {
        const detailed = [];
        for (const parameter of parameters.slice(0, 60)) {
            if (!parameter.conceptUuid) {
                detailed.push(parameter);
            ...
            const concept = await this.fetchOptional(target, `/concept/${encodeURIComponent(parameter.conceptUuid)}?v=custom:(uuid,display,datatype,names,answers:(uuid,display,names),attributes:(uuid,display,voided,value,attributeType:(uuid,display)))`, headers, allowInsecureTls, timeout, true, warnings, `concept ${parameter.conceptUuid}`);
            if (!concept) {
                detailed.push(parameter);
                continue;
            }
    ...
    extractAnswers(concept) {
        const answers = Array.isArray(concept?.answers) ? concept.answers : [];
        return answers
            .filter((answer) => answer?.uuid)
            .map((answer) => {
            const names = Array.isArray(answer.names) ? answer.names : [];
            const shortName = names.find((name) => name?.conceptNameType === 'SHORT')?.name ?? null;
            const analyzerCodes = names
                .map((name) => name?.name)
                .filter((name) => !!name && name !== 'LIS_CODED_ANSWERS')
    ...
    bestAnalyzerCode(concept) {
        const names = Array.isArray(concept?.names) ? concept.names : [];
        const shortName = names.find((name) => name?.conceptNameType === 'SHORT')?.name;
        const fullName = names.find((name) => name?.localePreferred)?.name;
        return String(shortName ?? fullName ?? concept?.display ?? concept?.name ?? concept?.uuid ?? '').trim();
    }
    firstObject(...values) {
        return values.find((value) => value && typeof value === 'object') ?? null;
...
exports.OpenMrsLisMetadataService = OpenMrsLisMetadataService;
