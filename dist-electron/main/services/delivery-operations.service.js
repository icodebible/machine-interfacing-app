...
exports.DeliveryOperationsService = void 0;
...
const delivery_audit_service_1 = require("./delivery-audit.service");
...
class DeliveryOperationsService {
    ...
    mappings = new mappings_service_1.MappingsService();
    secrets = new target_secrets_service_1.TargetSecretsService();
    audit = new delivery_audit_service_1.DeliveryAuditService();
    async sendNow(queueId, operation = 'SEND_NOW') {
        ...
        const validation = this.mappings.validate(target.type);
        if (!validation.ok) {
            throw new Error(`Mapping validation failed: ${(validation.errors ?? validation.warnings ?? []).join(' | ')}`);
        }
        const preview = this.preview.previewFromQueue(queueId);
        if (!preview?.payload) {
            throw new Error('Preview payload could not be generated');
