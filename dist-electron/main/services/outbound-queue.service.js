...
class OutboundQueueService {
    ...
    enqueueNormalizedResult(normalizedResultId, targetId, options = {}) {
        ...
        db.prepare(`
            ...
            `).run(id, normalizedResultId, targetId, JSON.stringify(preview.payload ?? {}), preview.previewName ?? null, preview.sourceDocument ? JSON.stringify(preview.sourceDocument) : null, JSON.stringify(previewWarnings), JSON.stringify(previewErrors), JSON.stringify(preview.summary ?? null), initialStatus, lastError, ts, ts, actor.userId, actor.username, actor.userId, actor.username);
        return true;
    }
    requeue(queueId) {
        const db = (0, db_1.getDb)();
        ...
        if ((preview.errors ?? []).length > 0) {
            throw new Error(`Cannot requeue result because the configured transformation still has ${preview.errors?.length} blocking error${preview.errors?.length === 1 ? '' : 's'}: ${(preview.errors ?? []).join(' | ')}`);
        }
        const res = db
            .prepare(`
                UPDATE outbound_queue
                ...
                WHERE id = ?
                `)
            .run(JSON.stringify(preview.payload ?? {}), preview.previewName ?? null, preview.sourceDocument ? JSON.stringify(preview.sourceDocument) : null, JSON.stringify(preview.warnings ?? []), JSON.stringify(preview.errors ?? []), JSON.stringify(preview.summary ?? null), nowIso(), actor.userId, actor.username, queueId);
        if (res.changes !== 1)
            throw new Error('Queue item not found');
        return true;
    }
