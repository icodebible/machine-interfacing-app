...
class OutboundQueueService {
    ...
    enqueueNormalizedResult(normalizedResultId, targetId, options = {}) {
        ...
        db.prepare(`
            ...
            `).run(id, normalizedResultId, targetId, JSON.stringify(preview.payload ?? {}), preview.previewName ?? null, preview.sourceDocument ? JSON.stringify(preview.sourceDocument) : null, JSON.stringify(preview.warnings ?? []), JSON.stringify(preview.errors ?? []), JSON.stringify(preview.summary ?? null), deliveryStatus, ts, ts, actor.userId, actor.username, actor.userId, actor.username);
        return true;
    }
    requeue(queueId) {
        const db = (0, db_1.getDb)();
    ...
    rebuildPayload(queueId) {
        ...
        db.prepare(`
                ...
                SET payload_json = ?,
                    ...
                    updated_by_username = ?
                WHERE id = ?
            `).run(JSON.stringify(preview.payload ?? {}), preview.previewName ?? null, preview.sourceDocument ? JSON.stringify(preview.sourceDocument) : null, JSON.stringify(preview.warnings ?? []), JSON.stringify(previewErrors), JSON.stringify(preview.summary ?? null), deliveryStatus, previewErrors.length ? previewErrors.join('; ') : null, ts, actor.userId, actor.username, queueId);
        return preview;
    }
    retry(queueId) {
