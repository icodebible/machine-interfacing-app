...
function assertSafeBaseUrl(value) {
    ...
    try {
        ...
        if (error?.message?.includes('Do not put') || error?.message?.includes('Sensitive URL'))
            throw error;
...
class TargetsService {
    ...
    buildDeliveryEndpoint(target) {
        ...
        switch (target.type) {
            ...
            case 'OPENMRS':
                return `${base}/lab-results`;
            case 'LIS':
                if (base.endsWith('/ws/rest/v1'))
                    return `${base}/lab/multipleresults`;
                if (base.endsWith('/openmrs'))
                    return `${base}/ws/rest/v1/lab/multipleresults`;
    ...
    delete(id) {
        ...
        audit_service_1.auditService.record({
            ...
            actor: (0, actor_context_service_1.getCurrentActorStamp)(),
        });
        return true;
    }
    test(id) {
