...
class UsersService {
    ...
    async create(dto) {
        ...
        const ts = nowIso();
        const hash = await (0, auth_service_1.hashPasswordValue)(password);
        db.prepare(`INSERT INTO users (id, username, password_hash, must_change_password, is_active, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)`).run(id, username, hash, dto.must_change_password ?? 1, dto.is_active ?? 1, ts, ts);
    ...
    update(id, dto) {
        const db = (0, db_1.getDb)();
        const current = db.prepare(`SELECT * FROM users WHERE id = ? LIMIT 1`).get(id);
        if (!current)
        ...
        db.prepare(`UPDATE users
            SET username = COALESCE(?, username),
                ...
                updated_at = ?
            WHERE id = ?`).run(username, dto.must_change_password ?? null, dto.is_active ?? null, nowIso(), id);
        if (dto.role_ids)
            this.replaceUserRoles(id, dto.role_ids);
    ...
    async resetPassword(id, newPassword) {
        ...
        if (!newPassword || newPassword.length < 4)
            throw new Error('Password must contain at least 4 characters');
        const hash = await (0, auth_service_1.hashPasswordValue)(newPassword);
        db.prepare(`UPDATE users
            SET password_hash = ?, must_change_password = 1, updated_at = ?
            WHERE id = ?`).run(hash, nowIso(), id);
        (0, auditLog_1.logEvent)({
            level: 'warn',
            source: 'AUTH',
            entityType: 'user',
    ...
    authoritiesForUser(userId) {
        const db = (0, db_1.getDb)();
        const rows = db
            .prepare(`SELECT DISTINCT ra.authority_code
                FROM user_roles ur
