...
exports.MachineSimulationUseCaseService = void 0;
...
class MachineSimulationUseCaseService {
    ensureTable() {
        ...
        const legacyCobas = db.prepare(`
            SELECT id, sample_message
            FROM machine_simulation_use_cases
            WHERE code = 'COBAS_HPV_HL7_FINAL' AND machine_id IS NULL
            LIMIT 1
        `).get();
        if (legacyCobas?.id && String(legacyCobas.sample_message ?? '').includes('OBX|1|CE|HPV16')) {
            db.prepare(`
                UPDATE machine_simulation_use_cases
                SET description = ?, test_order_code = ?, test_order_name = ?, protocol = ?, message_type = ?,
                    sample_message = ?, variables_json = ?, expected_codes_json = ?, updated_at = ?
                WHERE id = ?
            `).run('Captured-real COBAS 6800 HPV final-result structure. Only patient HPV OBXs with POS, NEG, or NA are reportable.', '71432-9', 'Human Papilloma Virus DNA Test', 'HL7', 'OUL^R22', cobas_hpv_fixture_1.COBAS_HPV_ACTUAL_TEMPLATE, safeJson({ sampleId: cobas_hpv_fixture_1.COBAS_HPV_SIMULATION_SAMPLE_ID, messageControlId: 'HPV-SIM-ACTUAL-0001', messageDateTime: '20260828154346', observedAt: '20260828124316', hrhpvResult: 'NEG', hpv16Result: 'NEG', hpv18Result: 'NEG' }), safeJson(['HRHPV', 'HPV16', 'HPV18']), nowIso(), legacyCobas.id);
        }
    }
    list(machineId) {
        this.ensureTable();
    ...
    save(input) {
        ...
        if (existing) {
            db.prepare(`
                    ...
                    WHERE id = ?
                `).run(input.machine_id ?? null, code, input.name, input.description ?? null, input.test_order_code ?? null, input.test_order_name ?? null, input.protocol, input.message_type, input.sample_message, variablesJson, expectedCodesJson, input.status ?? 'ACTIVE', input.sort_order ?? 100, ts, actor.userId, actor.username, id);
        }
        else {
            db.prepare(`
                ...
                `).run(id, input.machine_id ?? null, code, input.name, input.description ?? null, input.test_order_code ?? null, input.test_order_name ?? null, input.protocol, input.message_type, input.sample_message, variablesJson, expectedCodesJson, input.status ?? 'ACTIVE', input.sort_order ?? 100, ts, ts, actor.userId, actor.username, actor.userId, actor.username);
        }
    ...
    delete(id) {
        this.ensureTable();
        (0, db_1.getDb)().prepare(`DELETE FROM machine_simulation_use_cases WHERE id = ?`).run(id);
        audit_service_1.auditService.record({
            source: 'SIMULATION',
            category: 'SIMULATION',
    ...
    recordRun(row) {
        ...
        db.prepare(`
            ...
            `).run(id, row.machine_id, row.use_case_id ?? null, row.use_case_name ?? null, row.protocol, row.message_type, row.payload_preview ?? null, row.result_status, row.result_message ?? null, row.logs_json ?? null, nowIso(), actor.userId, actor.username);
        audit_service_1.auditService.record({
            source: 'SIMULATION',
            category: 'SIMULATION',
    ...
    seedDefaults() {
        ...
        const defaults = [
            {
                ...
                description: 'Captured-real COBAS 6800 HPV final-result structure. Only patient HPV OBXs with POS, NEG, or NA are reportable.',
                test_order_code: '71432-9',
                test_order_name: 'Human Papilloma Virus DNA Test',
                protocol: 'HL7',
                message_type: 'OUL^R22',
                expected_codes: ['HRHPV', 'HPV16', 'HPV18'],
                variables: {
                    sampleId: cobas_hpv_fixture_1.COBAS_HPV_SIMULATION_SAMPLE_ID,
                    messageControlId: 'HPV-SIM-ACTUAL-0001',
                    messageDateTime: '20260828154346',
                    observedAt: '20260828124316',
                    hrhpvResult: 'NEG',
                    hpv16Result: 'NEG',
                    hpv18Result: 'NEG',
                },
                sample_message: cobas_hpv_fixture_1.COBAS_HPV_ACTUAL_TEMPLATE,
                sort_order: 10,
            },
            {
        ...
        for (const item of defaults) {
            ...
            const exists = code
                ? db.prepare(`SELECT id FROM machine_simulation_use_cases WHERE code = ? AND machine_id IS NULL LIMIT 1`).get(code)
                : undefined;
            if (!exists?.id)
                this.save(item);
        }
    ...
    variablesFor(row) {
        return parseJson(row.variables_json, {});
    }
    expectedCodesFor(row) {
        return parseJson(row.expected_codes_json, []);
    }
}
exports.MachineSimulationUseCaseService = MachineSimulationUseCaseService;
