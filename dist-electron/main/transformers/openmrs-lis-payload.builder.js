...
exports.buildOpenMrsLisMultipleResultsPayload = buildOpenMrsLisMultipleResultsPayload;
// type MappingRuleRow = {
//     id: string;
//     target_type: string;
//     source_field: string;
...
function buildOpenMrsLisMultipleResultsPayload(args) {
...
class OpenMrsLisPayloadBuilder {
    ...
    blockedExistingCount = 0;
    existingMatchCount = 0;
    correctionCount = 0;
    repeatCount = 0;
    compositeExpandedCount = 0;
    constructor(targetType, sourceDocument, target) {
    ...
    build() {
        ...
        for (const observation of observations) {
            ...
            if (!mappedConceptUuid) {
                this.errors.push(`No LIS concept UUID mapping found for analyzer code "${observation.code}".` +
            ...
            if (!codedUuid && numericValue == null && !isEmpty) {
                this.errors.push(`No LIS coded answer UUID mapping found for analyzer result "${observation.code}=${value}".` +
            ...
            if (!handlingDecision.shouldInclude)
                continue;
        ...
        return {
            ...
            summary: {
                ruleCount: translations.conceptRuleCount +
    ...
    evaluateResultHandling(row, observation) {
        ...
        if (existing.length)
            this.existingMatchCount += existing.length;
        if (mode === 'CORRECTION') {
            this.correctionCount += existing.length ? 1 : 0;
            if (existing.length) {
                summary.decision = 'READY_CORRECTION';
                this.warnings.push(`Correction result detected for ${label}. ${existing.length} existing LIS result(s) were found and the corrected value will be prepared as a new /lab/multipleresults row.`);
            }
        ...
        if (mode === 'REPEAT') {
            ...
            if (existing.length) {
                ...
                this.warnings.push(`Repeat/rerun result detected for ${label}. ${existing.length} existing LIS result(s) were found and this repeat result will be prepared as a new /lab/multipleresults row.`);
            }
            this.resultHandlingRows.push(summary);
            return { shouldInclude: true };
        }
        if (sameValueExisting.length) {
            ...
            this.warnings.push(`Skipped duplicate LIS result for ${label}; the same value already exists in LIS. Mark the message as repeat/rerun or correction if it must be submitted again.`);
            this.resultHandlingRows.push(summary);
        ...
        if (existing.length) {
            ...
            this.errors.push(`Existing LIS result found for ${label}, but the analyzer message is not marked as repeat/rerun or correction.`);
            this.resultHandlingRows.push(summary);
            return { shouldInclude: false };
        }
        this.resultHandlingRows.push(summary);
    ...
    resultHandlingSummary(originalRowCount) {
        return {
    ...
    extractExistingAllocations(sourceDocument) {
    ...
    resolveResultHandlingMode(observation) {
        const raw = this.sourceDocument?.raw ?? this.sourceDocument;
        const values = [
            observation?.resultIntent,
            observation?.resultMode,
            observation?.resultAction,
            observation?.obxStatus,
    ...
    readLisTranslations() {
        ...
        for (const row of translations) {
            if (!enabledRuleIds.has(row.mapping_rule_id))
                continue;
        ...
        const index = {
            conceptUuidByCode: new Map(),
            allocationUuidByCode: new Map(),
            codedUuidByResult: new Map(),
            directValuesByDestination: new Map(),
            conceptRuleCount: 0,
            codedRuleCount: 0,
    ...
    addTranslationRows(map, rows, mode) {
        for (const row of rows) {
            const source = String(row.source_value ?? '').trim();
            const destinationUuid = this.extractUuid(row.destination_value);
            if (!source || !destinationUuid)
                continue;
            const keys = mode === 'result' ? this.resultKeyAliases(source) : this.codeAliases(source);
            for (const key of keys)
                map.set(key, destinationUuid);
        }
    }
    extractAnalyzerObservations(source) {
        const rawObservations = [];
        const add = (candidate, parentCode) => {
    ...
    toObservation(candidate, parentCode) {
        if (candidate == null)
            return null;
        ...
        if (typeof candidate !== 'object')
            return null;
        const code = this.firstText(candidate.code, candidate.analyzerCode, candidate.testCode, candidate.test_code, candidate.identifier, candidate.obxCode, candidate.observationCode, candidate.loinc, candidate.conceptCode, candidate.concept?.code, candidate.concept?.display, candidate.test?.code, candidate.name);
        const value = this.firstExisting(candidate.value, candidate.result, candidate.resultValue, candidate.observationValue, candidate.valueCoded?.display, candidate.valueCoded?.name, candidate.valueText, candidate.valueNumeric, candidate.rawValue);
    ...
    expandCompositeObservation(observation) {
    ...
    lookupConceptUuid(observation, translations) {
    ...
    lookupMappedAllocationUuid(observation, translations) {
        for (const alias of this.observationCodeAliases(observation)) {
    ...
    lookupCodedAnswerUuid(observation, translations) {
        const value = this.valueText(observation.value);
        if (!value)
            return null;
        const keys = [
    ...
    observationCodeAliases(observation) {
        return this.unique([
    ...
    codeAliases(raw) {
        ...
        return this.unique([upper, withoutPrefix, relaxed, hrhpv].filter(Boolean));
    }
    resultKeyAliases(raw) {
        const text = String(raw ?? '').trim();
        if (!text)
            return [];
        const parsed = this.parseKeyValue(text);
        if (!parsed)
            return this.valueAliases(text);
    ...
    valueAliases(raw) {
        ...
        if (normalized === 'INVALID')
            aliases.push('INV');
        if (normalized === 'POS')
            aliases.push('POSITIVE');
        if (normalized === 'NEG')
            aliases.push('NEGATIVE');
        if (normalized === 'INV')
            aliases.push('INVALID');
        return this.unique(aliases.filter(Boolean));
    }
    extractAllocationIndex(source) {
        const allocationByConceptUuid = new Map();
        const visit = (node, parent = null) => {
            ...
            if (typeof node !== 'object')
                return;
            const conceptUuid = this.firstText(node.concept?.uuid, node.parameter?.uuid, node.testParameter?.uuid, node.parameterConcept?.uuid, node.conceptUuid, node.parameterUuid);
            const allocationUuid = this.firstText(node.testAllocation?.uuid, node.allocation?.uuid, node.uuid, parent?.testAllocation?.uuid, parent?.allocation?.uuid);
    ...
    isAllocationDestination(destination) {
    ...
    isCodedAnswerDestination(destination) {
