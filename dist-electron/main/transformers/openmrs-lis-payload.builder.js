...
exports.buildOpenMrsLisPayload = buildOpenMrsLisPayload;
...
function buildOpenMrsLisPayload(target, sourceDocument) {
    if (target.type !== 'LIS') {
    ...
    const constants = {
        ...
        statusRemarks: constantFor(rules, DESTINATION_ALIASES.statusRemarks) ??
    ...
    for (const obs of observations) {
        ...
        const conceptUuid = firstNonEmpty(obs.target?.conceptUuid, obs.targetConceptUuid, lookupMap(conceptMap, [sourceCode, codeKey]));
        const allocationUuid = firstNonEmpty(obs.target?.testAllocationUuid, obs.testAllocationUuid, obs.targetAllocationUuid, lookupMap(allocationMap, [sourceCode, codeKey]));
        const datatype = String(firstNonEmpty(obs.target?.datatype, obs.targetDatatype, lookupMap(datatypeMap, [sourceCode, codeKey]), inferDatatype(obs.valueType)) ?? 'text').toLowerCase();
        const rowWarnings = [];
        if (!conceptUuid)
            rowWarnings.push(`No LIS concept UUID mapping found for analyzer code "${sourceCode}".`);
        if (!allocationUuid) {
        ...
        if (datatype.includes('coded') || ['ce', 'cwe', 'cw'].includes(String(obs.valueType ?? '').toLowerCase())) {
            const valueCodedUuid = firstNonEmpty(obs.target?.valueCodedUuid, obs.valueCodedUuid, lookupMap(valueCodedMap, [
            ...
            if (valueCodedUuid) {
                ...
                rowWarnings.push(`No LIS coded answer UUID mapping found for analyzer result "${sourceCode}=${sourceValue}".`);
            }
        }
            ...
            if (Number.isFinite(numericValue)) {
                ...
                rowWarnings.push(`Analyzer result "${sourceCode}" was expected to be numeric but value "${sourceValue}" was not numeric; it was staged as text.`);
            }
        }
        else {
            row.valueText = sourceValue == null ? null : String(sourceValue);
    ...
    if (!body.length) {
        errors.push('No LIS multiple-results rows are ready. Configure LIS mappings for analyzer code → concept UUID, analyzer code → allocation UUID, and analyzer value → coded answer UUID where applicable.');
    }
    return {
        used: body.length > 0,
        warnings,
...
function constantFor(rules, destinationAliases) {
    const rule = rules.find((item) => item.transform_kind === 'constant' && destinationAliases.includes(item.destination_field));
    return parseScalar(rule?.constant_value ?? null);
}
function buildDestinationMap(rules, translationMap, destinationAliases) {
    const map = new Map();
...
function lookupMap(map, candidates) {
...
function getObservations(sourceDocument) {
    const candidates = [
...
function inferDatatype(valueType) {
    const value = String(valueType ?? '').trim().toUpperCase();
    if (value === 'NM' || value === 'SN')
        return 'numeric';
    if (value === 'CE' || value === 'CWE' || value === 'CW')
        return 'coded';
    return 'text';
}
function isAbnormal(flag) {
    const value = String(flag ?? '').trim().toUpperCase();
    if (!value || value === 'N' || value === 'NORMAL')
        return false;
    return true;
}
