...
class TargetTransformPreviewService {
    preview(targetId, normalizedResultId) {
        ...
        if (target.type === 'LIS') {
            ...
            if (!hasPayload && defaultLisPayload) {
                warnings.unshift('No LIS multiple-results rows were generated from the current mappings. Showing the built-in fallback payload generated from the normalized result.');
            }
            return {
                targetId: target.id,
                targetType: target.type,
    ...
    renderMappings(targetType, sourceDocument, config = this.loadMappingConfig(targetType)) {
    ...
    buildLisSourceDocument(rawData, result) {
        ...
        return {
            ...
            defaultRemarks: lis?.defaultRemarks ?? 'Machine interfaced result',
        };
    }
    buildLisRowsFromResults(results) {
        ...
        return results
            ...
            status: {
                ...
                status: 'REMARKS',
                remarks: 'Machine interfaced result',
            },
