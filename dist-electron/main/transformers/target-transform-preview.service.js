...
exports.TargetTransformPreviewService = void 0;
const db_1 = require("../db/db");
const openmrs_lis_payload_builder_1 = require("./openmrs-lis-payload.builder");
class TargetTransformPreviewService {
    preview(targetId, normalizedResultId) {
        ...
        if (target.type === 'LIS') {
            const lisBuild = (0, openmrs_lis_payload_builder_1.buildOpenMrsLisMultipleResultsPayload)({
            ...
            return {
                ...
                previewName: `${target.name || target.type} · OpenMRS LIS multiple-results preview`,
                payload: lisBuild.payload ?? sourceDocument,
                sourceDocument,
                warnings: lisBuild.warnings,
                errors: lisBuild.errors,
                summary: lisBuild.summary,
                payloadSource: 'regenerated',
            };
        }
        const rendered = this.renderMappings(target.type, sourceDocument);
        if (rendered.summary.ruleCount === 0) {
            rendered.warnings.unshift(`No enabled mapping rules were found for ${target.type}. Returning the canonical source document as the preview payload.`);
        }
        return {
            targetId: target.id,
            targetType: target.type,
            ...
            payload: rendered.summary.ruleCount > 0 && Object.keys(rendered.payload).length > 0
                ? rendered.payload
                : sourceDocument,
            sourceDocument,
    ...
    previewFromDeliveryHistory(queueId) {
    ...
    previewFromStoredPayload(queueRow, fallbackName, delivered) {
        const target = this.getTarget(queueRow.target_id);
        const deliveredPayload = delivered ? this.tryParseJson(queueRow.delivered_payload_json) : null;
        const storedPayload = deliveredPayload ?? this.tryParseJson(queueRow.payload_json);
        if (storedPayload && typeof storedPayload === 'object') {
            if (Array.isArray(storedPayload) || !this.looksLikeLegacyFlatPayload(storedPayload)) {
                const warnings = [
                    delivered
                        ? deliveredPayload
                            ? 'Preview is based on the exact dynamically enriched payload delivered to the target.'
        ...
        if (storedPayload && !Array.isArray(storedPayload) && this.looksLikeLegacyFlatPayload(storedPayload)) {
            warnings.unshift('Stored payload uses a legacy flat-key mapping snapshot. Preview was regenerated with the current nested renderer.');
        }
        else {
            warnings.unshift('Stored payload metadata was unavailable or incomplete, so the preview was regenerated with the current mapping engine.');
        }
        return {
            ...regenerated,
            warnings,
    ...
    renderMappings(targetType, sourceDocument) {
        ...
        for (const rule of rules) {
            ...
            const resolved = this.resolveRuleValue(rule, sourceDocument, translationMap.get(rule.id) ?? [], ctx);
            if (resolved.skip) {
                ctx.summary.skippedCount += 1;
                continue;
            }
    ...
    resolveRuleValue(rule, sourceDocument, translations, ctx) {
        let resolved;
        ...
        switch (String(rule.unmapped_behavior ?? 'PASSTHROUGH')) {
            ...
            case 'ERROR':
                ctx.errors.push(`Rule ${rule.id}: source path "${String(rule.source_field ?? '').trim()}" did not resolve a value.`);
                return { value: null, skip: true, translated: false };
            case 'PASSTHROUGH':
            default:
                ctx.warnings.push(`Rule ${rule.id}: source path "${String(rule.source_field ?? '').trim()}" did not resolve a value; null was assigned.`);
                return { value: null, skip: false, translated: false };
        }
    }
    applyValueTranslation(rule, rawValue, rows, ctx) {
        if (Array.isArray(rawValue)) {
            let translated = false;
            const values = rawValue.map((item) => {
                const next = this.applyValueTranslation(rule, item, rows, ctx);
                if (next.kind === 'ok' && next.translated)
                    translated = true;
        ...
        switch (String(rule.unmapped_behavior ?? 'PASSTHROUGH')) {
            ...
            case 'ERROR':
                ctx.errors.push(`Rule ${rule.id}: no value translation matched source value "${rawKey}" for destination "${rule.destination_field}".`);
                return { kind: 'skip' };
            case 'PASSTHROUGH':
            default:
    ...
    buildCanonicalSourceDocument(result, target) {
    ...
    buildLisSourceDocument(rawData, result) {
        ...
        return {
            ...
            instrument: rawData?.instrument ?? rows.find((row) => row?.instrument)?.instrument ?? null,
            defaultRemarks: lis?.defaultRemarks ?? 'Machine interfaced result 1',
        };
    }
    buildLisRowsFromResults(results) {
    ...
    applyLisResultHandlingMessages(rendered, sourceDocument, defaultPayload) {
        ...
        for (const warning of warnings) {
            if (!rendered.warnings.includes(warning))
                rendered.warnings.push(warning);
        }
        for (const error of errors) {
            if (!rendered.errors.includes(error))
                rendered.errors.push(error);
        }
        const summary = rendered.summary && typeof rendered.summary === 'object' ? rendered.summary : {};
    ...
    extractLisResultHandling(sourceDocument, defaultPayload) {
        ...
        if (fromSource && typeof fromSource === 'object')
            return fromSource;
        return null;
    }
    resolveLisResultHandling(rows, rawData) {
        ...
        for (let index = 0; index < inputRows.length; index += 1) {
            ...
            const sameValueExisting = existing.filter((result) => this.lisRowValueMatchesExistingResult(row, result));
            ...
            if (mode === 'CORRECTION') {
                ...
                if (existing.length) {
                    warnings.push(`Correction result detected for ${rowSummary.label}. ${existing.length} existing LIS result(s) were found and the new corrected value will be prepared as a new /lab/multipleresults row.`);
            ...
            if (mode === 'REPEAT') {
                ...
                if (existing.length) {
                    warnings.push(`Repeat/rerun result detected for ${rowSummary.label}. ${existing.length} existing LIS result(s) were found and this repeat result will be prepared as a new /lab/multipleresults row.`);
            ...
            if (sameValueExisting.length) {
                ...
                warnings.push(`Skipped duplicate LIS result for ${rowSummary.label}; the same value already exists in LIS. Mark the message as repeat/rerun or correction if it must be submitted again.`);
                rowSummaries.push(rowSummary);
                continue;
            }
            if (existing.length) {
                ...
                errors.push(`Existing LIS result found for ${rowSummary.label}, but the incoming analyzer message is not marked as repeat/rerun or correction. Rebuild after confirming the result intent.`);
                rowSummaries.push(rowSummary);
                continue;
            }
        ...
        if (inputRows.length > 0 && readyRows.length === 0) {
            errors.push('No LIS result rows remain after duplicate/repeat/correction handling. Review existing LIS results or mark the analyzer message as repeat/rerun or correction where appropriate.');
        }
        return {
            rows: readyRows,
            summary: {
    ...
    findLisAllocationCandidates(rawData) {
        const candidates = [
    ...
    findExistingLisResultsForRow(row, allocations) {
        ...
        if (!allocations.length || (!allocationUuid && !conceptUuid))
            return [];
        const existing = [];
        for (const allocation of allocations) {
            const candidateAllocationUuid = String(allocation?.uuid ?? allocation?.testAllocation?.uuid ?? '').trim();
            const candidateConceptUuid = String(allocation?.concept?.uuid ?? allocation?.parameter?.uuid ?? allocation?.testParameter?.uuid ?? '').trim();
            ...
            if (!allocationMatches && !conceptMatches)
                continue;
            ...
            for (const result of results) {
                ...
                if (conceptUuid && resultConceptUuid && resultConceptUuid !== conceptUuid)
                    continue;
    ...
    resolveLisResultMode(row, rawData) {
        const values = [
            row?.resultIntent,
            row?.resultMode,
            row?.resultAction,
    ...
    buildLisHandlingRowSummary(row, index, mode, existing, sameValueExisting) {
    ...
    describeLisPayloadRow(row, index) {
    ...
    buildDefaultLisPayload(sourceDocument) {
    ...
    setByPath(target, path, value) {
        ...
        for (let i = 0; i < tokens.length; i += 1) {
            ...
            if (!Array.isArray(cursor)) {
                throw new Error(`Destination path "${path}" tries to address an array segment on a non-array value.`);
            }
    ...
    looksLikeLegacyFlatPayload(payload) {
    ...
    getNormalizedResult(normalizedResultId) {
        const db = (0, db_1.getDb)();
        const result = db
            .prepare(`
                    SELECT *
        ...
        if (!result)
            throw new Error('Normalized result not found');
        return result;
    }
    getQueueRow(queueId, errorMessage) {
        const db = (0, db_1.getDb)();
        const queueRow = db
            .prepare(`
                    SELECT *
...
exports.TargetTransformPreviewService = TargetTransformPreviewService;
