...
var IPC_CHANNELS = {
  ...
  MACHINES_LOGS_HOST_INFO: "machines:logs:host-info",
  MACHINES_LOGS_OPEN_FOLDER: "machines:logs:open-folder",
  MACHINES_LOGS_OPEN_RAW_FOLDER: "machines:logs:open-raw-folder",
...
var api = {
  ...
  machinesLogsReplay: (logId, mode) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.MACHINES_LOGS_REPLAY, logId, mode),
  machinesLogsHostInfo: (machineId) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.MACHINES_LOGS_HOST_INFO, machineId),
  machinesLogsOpenFolder: (machineId) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.MACHINES_LOGS_OPEN_FOLDER, machineId),
  machinesLogsOpenRawFolder: (machineId) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.MACHINES_LOGS_OPEN_RAW_FOLDER, machineId),
  machinesRuntimeSessionsList: (machineId, limit) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.MACHINES_SESSIONS_LIST, machineId, limit),
  ...
  machinesSimUseCasesList: (machineId) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.MACHINES_SIM_USE_CASES_LIST, machineId),
  machinesSimUseCaseSave: (dto) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.MACHINES_SIM_USE_CASE_SAVE, dto),
  machinesSimUseCaseDelete: (id) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.MACHINES_SIM_USE_CASE_DELETE, id),
  machinesSimUseCaseRun: (machineId, useCaseId, variables) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.MACHINES_SIM_USE_CASE_RUN, machineId, useCaseId, variables),
  machinesSimRunHistory: (machineId, limit) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.MACHINES_SIM_RUN_HISTORY, machineId, limit),
  ...
  outboundQueueRebuildPayload: (queueId) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.OUTBOUND_QUEUE_REBUILD_PAYLOAD, queueId),
  ...
  auditEventsQuery: (q = {}) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.AUDIT_EVENTS_QUERY, q),
  auditEventsSummary: (days) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.AUDIT_EVENTS_SUMMARY, days),
  deploymentReadinessCheck: () => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.DEPLOYMENT_READINESS_CHECK),
  appDiagnosticsGet: () => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.APP_DIAGNOSTICS_GET),
  databaseBackupCreate: () => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.APP_DATABASE_BACKUP_CREATE),
  ...
  lisTestOrderProfilesList: (targetId) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.LIS_TEST_ORDER_PROFILES_LIST, targetId ?? null),
  lisTestOrderProfileGet: (id) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.LIS_TEST_ORDER_PROFILES_GET, id),
  lisTestOrderProfileSave: (payload) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.LIS_TEST_ORDER_PROFILES_SAVE, payload),
  lisTestOrderProfileSetEnabled: (id, enabled) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.LIS_TEST_ORDER_PROFILES_SET_ENABLED, id, enabled),
  lisTestOrderProfileDelete: (id) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.LIS_TEST_ORDER_PROFILES_DELETE, id),
  ...
  routingRulesList: () => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.ROUTING_RULES_LIST),
  routingRulesCreate: (payload) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.ROUTING_RULES_CREATE, payload),
  routingRulesUpdate: (id, patch) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.ROUTING_RULES_UPDATE, id, patch),
  routingRulesDelete: (id) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.ROUTING_RULES_DELETE, id),
  routingRulesPreviewResult: (normalizedResultId, fallbackTargetIds = []) => import_electron2.ipcRenderer.invoke(IPC_CHANNELS.ROUTING_RULES_PREVIEW_RESULT, normalizedResultId, fallbackTargetIds),
  // preload index.ts
