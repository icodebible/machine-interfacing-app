...
//       return searchMatch && queueMatch && targetMatch && operationMatch && statusMatch && fromMatch && toMatch;
//     });
//   });
...
//       lisRows: this.previewReadyRows(row),
//       errors: this.previewErrorCount(row),
//       warnings: this.previewWarningCount(row),
//     };
//   }


//   readinessClass(row: any) {
//     const preview = this.previewFor(row?.id);
//     if (!row?.queue_id) return 'idle';
//     if (!preview) return 'idle';
//     if (this.previewErrors(preview).length > 0) return 'bad';
//     if (this.previewWarnings(preview).length > 0) return 'warn';
//     const rows = this.previewPayloadRows(preview);
//     return rows.length > 0 ? 'ok' : 'warn';
//   }

//   readinessLabel(row: any) {
//     const preview = this.previewFor(row?.id);
//     if (!row?.queue_id) return 'No queue context';
//     if (!preview) return 'Preview not loaded';

//     const errors = this.previewErrors(preview).length;
//     const warnings = this.previewWarnings(preview).length;
//     const rows = this.previewPayloadRows(preview).length;

//     if (errors > 0) return `${errors} blocking issue${errors === 1 ? '' : 's'}`;
//     if (rows > 0 && warnings === 0) return `${rows} LIS row${rows === 1 ? '' : 's'} ready`;
//     if (rows > 0) return `${rows} row${rows === 1 ? '' : 's'} with warning${warnings === 1 ? '' : 's'}`;
//     return 'Needs mapping review';
//   }

//   previewErrorCount(row: any) {
//     return this.previewErrors(this.previewFor(row?.id)).length;
//   }

//   previewWarningCount(row: any) {
//     return this.previewWarnings(this.previewFor(row?.id)).length;
//   }

//   previewReadyRows(row: any) {
//     return this.previewPayloadRows(this.previewFor(row?.id)).length;
//   }

//   previewTopMessages(row: any) {
//     const preview = this.previewFor(row?.id);
//     const errors = this.previewErrors(preview).map((text) => ({ kind: 'bad' as const, text }));
//     const warnings = this.previewWarnings(preview).map((text) => ({ kind: 'warn' as const, text }));
//     return [...errors, ...warnings].slice(0, 3);
//   }

//   private previewErrors(preview: any): string[] {
//     if (!preview) return [];
//     return this.asStringArray(
//       preview.errors ??
//       preview.errorMessages ??
//       preview.validation?.errors ??
//       preview.readiness?.errors ??
//       preview.summary?.errors,
//     );
//   }

//   private previewWarnings(preview: any): string[] {
//     if (!preview) return [];
//     return this.asStringArray(
//       preview.warnings ??
...
//   private previewPayloadRows(preview: any): any[] {
//     if (!preview) return [];
//     const candidates = [
//       preview.payload,
//       preview.previewPayload,
//       preview.transformedPayload,
//       preview.requestBody,
//       preview.body,
//       preview.lisPayload,
//       preview.openMrsPayload,
//     ];
...
//       return value
//         .map((item) => typeof item === 'string' ? item : item?.message ?? item?.text ?? JSON.stringify(item))
//         .filter((item) => !!String(item ?? '').trim())
//         .map((item) => String(item));
//     }
//     if (typeof value === 'string') return value.trim() ? [value] : [];
//     if (typeof value === 'object') {
//       const text = value.message ?? value.text ?? JSON.stringify(value);
...
export class DeliveryHistory {
  ...
  constructor() {
    const queueIdFromRoute = this.route.snapshot.queryParamMap.get('queueId');
    if (queueIdFromRoute) {
      this.queueId = queueIdFromRoute;
  ...
  async refresh() {
    try {
      ...
      const [deliveredRows, auditRows] = await Promise.all([
        this.api.deliveryHistoryList(300),
        this.api.deliveryAuditQuery({
          queueId: this.queueId.trim() || null,
          targetId: this.targetId.trim() || null,
  ...
  readinessClass(row: any): 'ok' | 'warn' | 'bad' | 'idle' {
    const preview = this.previewFor(row?.id);
    if (!row?.queue_id) return 'idle';
    if (!preview) return 'idle';
    if (this.previewErrors(preview).length > 0) return 'bad';
  ...
  previewErrorCount(row: any): number {
    return this.previewErrors(this.previewFor(row?.id)).length;
  }

  previewWarningCount(row: any): number {
    return this.previewWarnings(this.previewFor(row?.id)).length;
  }

  previewReadyRows(row: any): number {
    return this.previewPayloadRows(this.previewFor(row?.id)).length;
  }

  previewTopMessages(row: any): Array<{ kind: 'info' | 'warn' | 'bad'; text: string }> {
    const preview = this.previewFor(row?.id);
    const errors: Array<{ kind: 'info' | 'warn' | 'bad'; text: string }> = this.previewErrors(preview)
      .map((text) => ({ kind: 'bad', text }));
    const warnings: Array<{ kind: 'info' | 'warn' | 'bad'; text: string }> = this.previewWarnings(preview)
      .map((text) => ({ kind: 'warn', text }));
    const messages = [...errors, ...warnings].slice(0, 3);

    if (messages.length > 0) return messages;
    if (preview) {
      return [{ kind: 'info', text: 'Latest preview has no blocking validation messages.' }];
    }
    return [{ kind: 'info', text: 'Open the row or preview workspace to load the latest payload readiness.' }];
  }
  ...
  private previewPayloadRows(preview: any): any[] {
    ...
    const candidates = [
      ...
      preview.openMrsPayload,
      preview.openmrsPayload,
      preview.metadata?.openmrsLis?.rows,
      preview.metadata?.lis?.rows,
    ];
