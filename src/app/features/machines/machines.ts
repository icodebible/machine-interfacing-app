...
type FilterEnabled = 'all' | 'enabled' | 'disabled';
type SimulationScenario = 'ASTM_BASIC' | 'HL7_ORU' | 'HL7_COBAS_HPV_FINAL_RESULT' | 'RAW_PING';
type FilterConn = 'all' | 'TCP' | 'SERIAL' | 'HL7_MLLP' | 'FTP' | 'SFTP' | 'FILE_WATCHER';
...
export class Machines {
  ...
  simulationStates = signal<Record<string, any>>({});
  simulationScenario = signal<SimulationScenario>('ASTM_BASIC');
  simulationIntervalMs = signal(5000);
  ...
  private simulationScenarioForMachine(row: any): SimulationScenario {
    const protocol = String(row?.protocol ?? '').toUpperCase();
    const connectionType = String(row?.connection_type ?? '').toUpperCase();
  ...
  async startSimulation(row: any) {
    try {
      await this.api.machinesSimStart(
        row.id,
        this.simulationScenarioForMachine(row),
        this.simulationIntervalMs(),
      );
      this.snack.open('Simulation started', 'OK', { duration: 1800 });
      await this.refreshSimulationStates();
