...
//   policies = signal<any[]>([]);
//   expandedRowId = signal<string | null>(null);
//   actionInProgressKey = signal<string | null>(null);
...
//     } finally {
//       this.loading.set(false);
//       this.actionInProgressKey.set(null);
//     }
//   }
...
//   currentActorActed(row: any) {
//     return !!this.currentActorDecision(row);
//   }

//   currentActorDecision(row: any) {
//     const currentId = String(this.approverUserId() ?? '').trim();
//     if (!currentId) return null;

//     const actions = Array.isArray(row?.approval_actions) ? row.approval_actions : [];
//     return (
//       actions.find((action: any) => String(action?.approver_user_id ?? '').trim() === currentId) ??
//       null
//     );
//   }

//   currentActorActionText(row: any) {
//     const action = String(this.currentActorDecision(row)?.action ?? '').toUpperCase();
...
//   isActionInProgress(row: any, action: ApprovalAction) {
//     return this.actionInProgressKey() === this.actionKey(row, action);
//   }

//   approvalActionTooltip(row: any, action: ApprovalAction) {
//     if (this.isActionInProgress(row, action)) {
//       return action === 'approve' ? 'Approving...' : 'Rejecting...';
//     }
//     if (this.currentActorActed(row)) return this.actionBlockedMessage(row);
//     if (!this.canAct(row)) return this.actionBlockedMessage(row);
//     return action === 'approve' ? 'Approve' : 'Reject';
//   }

//   actionBlockedMessage(row: any) {
//     if (this.currentActorActed(row)) {
//       return `You already ${this.currentActorActionText(row)} this result. Another approver is required if more approvals are still needed.`;
//     }
...
//   async openAction(row: any, action: ApprovalAction) {
//     if (!this.canAct(row)) {
//       this.snack.open(this.actionBlockedMessage(row), 'Close', { duration: 4200 });
//       return;
//     }
...
//     const actionKey = this.actionKey(row, action);
//     try {
//       this.actionInProgressKey.set(actionKey);
//       const dto = {
//         normalizedResultId: row.normalized_result_id,
//         policyId: row.approval_policy_id,
//         stepOrder: 1,
...
//       await this.refresh();
//     } finally {
//       if (this.actionInProgressKey() === actionKey) this.actionInProgressKey.set(null);
//     }
//   }
...
//     if (this.currentActorActed(row)) {
//       notes.push({ kind: 'info', text: this.actionBlockedMessage(row) });
//     }
...
//     if (row?.status === 'PENDING_APPROVAL' && !this.currentActorActed(row)) {
//       notes.push({
//         kind: 'info',
//         text: 'This result is ready for supervisor review and approval decision.',
//       });
...
//   private actionKey(row: any, action: ApprovalAction) {
//     return `${row?.normalized_result_id || row?.id || 'unknown'}:${action}`;
//   }
