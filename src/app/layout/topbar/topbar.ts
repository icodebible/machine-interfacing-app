...
export class Topbar {
  ...
  private readonly routeMeta: Record<string, RouteMeta> = {
    ...
    '/lis-test-order-profiles': {
      title: 'LIS Test-Order Profiles',
      subtitle: 'Manage one operational metadata profile per LIS test order.',
    },
    '/routing-rules': {
      title: 'Routing Rules',
      subtitle: 'Configure automatic target routing rules for approved results.',
    },
    '/outbound-queue': {
    ...
    '/audit-logs': {
      ...
      subtitle: 'Trace actions, inspect audit events, and review deployment readiness.',
    },
    '/users': {
      title: 'Users',
  ...
  constructor() {
    ...
    void this.loadApplicationPresentation();
  ...
  private async loadApplicationPresentation() {
    try {
      const diagnostics = await this.platform.appDiagnosticsGet();
      const version = diagnostics?.app?.version;
      if (version) this.appVersion.set(`v${version}`);
      this.appMode.set(diagnostics?.app?.isPackaged ? 'Packaged' : 'Development');
    } catch {
      try {
        const version = await this.platform.getAppVersion();
        if (version) this.appVersion.set(`v${version}`);
      } catch {
        // keep version hidden if preload is unavailable
      }
    }
  }
