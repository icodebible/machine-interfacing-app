...
export class OutboundQueueService {
    ...
    enqueueNormalizedResult(normalizedResultId: string, targetId: string, options: EnqueueOptions = {}) {
        const db = getDb();
        const actor = getCurrentActorStamp();
    ...
    private refreshQueuePayload(queueId: string, options: EnqueueOptions = {}) {
        const db = getDb();
        const actor = getCurrentActorStamp();
        const row = db
            .prepare(
            ...
            .get(queueId) as { id: string; normalizedResultId: string; targetId: string } | undefined;
        ...
        if (previewErrors.length > 0 && !options.allowInvalidPreview) {
            throw new Error(
                `Cannot send result because the latest payload preview has ${previewErrors.length} blocking error${previewErrors.length === 1 ? '' : 's'}: ${previewErrors.join('; ')}`,
            );
        }
        ...
        db.prepare(
            ...
            previewErrors.length > 0 ? 'FAILED' : 'PENDING',
            JSON.stringify(preview.payload ?? {}),
            preview.previewName ?? null,
            preview.sourceDocument ? JSON.stringify(preview.sourceDocument) : null,
            JSON.stringify(preview.warnings ?? []),
            JSON.stringify(previewErrors),
            JSON.stringify(preview.summary ?? null),
            previewErrors.length > 0 ? previewErrors.join('; ') : null,
            nowIso(),
            actor.userId,
            actor.username,
            queueId,
