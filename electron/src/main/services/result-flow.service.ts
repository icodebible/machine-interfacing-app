...
export class ResultFlowService {
  ...
  advanceAfterNormalization(normalizedResultId: string) {
    ...
    if (resolution.matchStatus === 'NO_MATCH' || !resolution.policy) {
      this.upsertWorkflow(normalizedResultId, {
        ...
        failed_at: null,
        last_error:
          'No matching approval policy was found for this result. Configure and enable a policy, then re-check policy from Pending Approvals.',
        updated_at: nowIso(),
    ...
    if (resolution.matchStatus === 'DISABLED_MATCH') {
      this.upsertWorkflow(normalizedResultId, {
        ...
        failed_at: null,
        last_error:
          'A matching approval policy exists but is disabled. Enable the policy, then re-check policy from Pending Approvals.',
        updated_at: nowIso(),
        created_by_user_id: actor.userId,
        created_by_username: actor.username,
        updated_by_user_id: actor.userId,
    ...
    if (!routeTargetIds.length) {
      this.upsertWorkflow(normalizedResultId, {
        ...
        failed_at: null,
        last_error:
          'Auto-route is allowed, but no routing targets are configured on the matched policy. Update the policy, then re-check it from Pending Approvals.',
        updated_at: nowIso(),
        created_by_user_id: actor.userId,
        created_by_username: actor.username,
        updated_by_user_id: actor.userId,
  ...
  queueApprovedResult(normalizedResultId: string, policyId: string) {
    ...
    if (!policy) {
      this.upsertWorkflow(normalizedResultId, {
        ...
        failed_at: null,
        last_error:
          'The linked approval policy could not be found. Routing is paused until policy configuration is restored.',
        updated_at: nowIso(),
        updated_by_user_id: actor.userId,
        updated_by_username: actor.username,
      });
    ...
    const required = Math.max(
      1,
      Number(workflow.approval_count_required ?? policy.min_approvals ?? 1),
    );
    ...
    if (policy.enabled !== 1) {
      this.upsertWorkflow(normalizedResultId, {
        ...
        failed_at: null,
        last_error:
          'The approval policy was disabled before routing could continue. Re-enable it to resume processing.',
        updated_at: nowIso(),
        updated_by_user_id: actor.userId,
        updated_by_username: actor.username,
      });
    ...
    if (!routeTargetIds.length) {
      this.upsertWorkflow(normalizedResultId, {
        ...
        failed_at: null,
        last_error:
          'Approval completed, but no routing targets are configured for this policy. Add targets and re-check policy from Pending Approvals.',
        updated_at: nowIso(),
        updated_by_user_id: actor.userId,
        updated_by_username: actor.username,
      });
  ...
  private getPolicyRouteTargetIds(policyId: string, policy?: any): string[] {
    const db = getDb();
    ...
    const linkedRows = db
      ...
      .all(policyId) as Array<{ target_id?: string | null }>;
  ...
  private queueTargets(normalizedResultId: string, targetIds: string[]): QueueTargetsResult {
    const db = getDb();
    ...
    const missingTargetIds = uniqueIds.filter(
  ...
  private getWorkflow(normalizedResultId: string) {
    const db = getDb();
    return db
  ...
  private getResultRoutingContext(normalizedResultId: string) {
    const db = getDb();
    const row = db
      .prepare(
        `
