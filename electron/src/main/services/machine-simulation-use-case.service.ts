...
import { COBAS_HPV_ACTUAL_TEMPLATE, COBAS_HPV_SIMULATION_SAMPLE_ID } from '../protocols/cobas-hpv.fixture';
...
export class MachineSimulationUseCaseService {
    ensureTable() {
        ...
        const legacyCobas = db.prepare(`
            ...
            LIMIT 1
        `).get() as { id: string; sample_message?: string | null } | undefined;
        if (legacyCobas?.id && String(legacyCobas.sample_message ?? '').includes('OBX|1|CE|HPV16')) {
            db.prepare(`
                ...
                WHERE id = ?
            `).run(
                'Captured-real COBAS 6800 HPV final-result structure. Only patient HPV OBXs with POS, NEG, or NA are reportable.',
                '71432-9',
                'Human Papilloma Virus DNA Test',
                'HL7',
                'OUL^R22',
                COBAS_HPV_ACTUAL_TEMPLATE,
                safeJson({ sampleId: COBAS_HPV_SIMULATION_SAMPLE_ID, messageControlId: 'HPV-SIM-ACTUAL-0001', messageDateTime: '20260828154346', observedAt: '20260828124316', hrhpvResult: 'NEG', hpv16Result: 'NEG', hpv18Result: 'NEG' }),
                safeJson(['HRHPV', 'HPV16', 'HPV18']),
                nowIso(),
                legacyCobas.id,
            );
        }
    ...
    seedDefaults(): void {
        ...
        const defaults: MachineSimulationUseCaseInput[] = [
            {
                ...
                variables: {
                    sampleId: COBAS_HPV_SIMULATION_SAMPLE_ID,
                ...
                sample_message: COBAS_HPV_ACTUAL_TEMPLATE,
                sort_order: 10,
            },
            {
        ...
        for (const item of defaults) {
            ...
            const exists = code
                ? db.prepare(`SELECT id FROM machine_simulation_use_cases WHERE code = ? AND machine_id IS NULL LIMIT 1`).get(code) as { id: string } | undefined
                : undefined;
            if (!exists?.id) this.save(item);
        }
    }
