...
type TargetLike = {
    id: string;
    type: 'DHIS2' | 'OPENMRS' | 'LIS' | 'CUSTOM_HTTP' | string;
    name: string;
    base_url: string;
    enabled: number;
};
...
export class DeliveryPreflightService {
    validate(args: {
        target: TargetLike;
        payload: Record<string, any>;
        mappingValidation: { ok: boolean; warnings?: string[] };
        secret?: SecretLike;
    }) {
        const errors: string[] = [];
        ...
        const authHeaders = buildAuthHeaders(secret);
        if (secret?.authType && secret.authType !== 'none' && Object.keys(authHeaders).length === 0) {
            errors.push(
                'Authentication headers could not be built from the stored secret configuration.',
            );
        }

        switch (args.target.type) {
            case 'DHIS2':
                if (!args.payload.eventDate) errors.push('DHIS2 payload is missing eventDate.');
            ...
            case 'OPENMRS':
                ...
                if (
                    args.payload.observation?.value === undefined ||
            ...
            case 'CUSTOM_HTTP':
                if (Array.isArray(args.payload)) {
                    warnings.push(
                        'Custom HTTP payload is an array. Confirm the remote endpoint expects a collection.',
                    );
                }
                break;
            ...
            default:
                warnings.push(
                    `No connector-specific preflight validator is defined for ${args.target.type}.`,
                );
                break;
        }
