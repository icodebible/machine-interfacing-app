...
export type OpenMrsLisTestOrderProfileSeed = {
  code: string;
  name: string;
  orderConceptUuid?: string | null;
  orderNameIncludes?: string[];
...
export type OpenMrsLisMappingSeedInput = {
  ...
  testOrderProfiles?: OpenMrsLisTestOrderProfileSeed[];
  parameters: Array<{
    analyzerCode: string;
...
export class MappingsService {
  ...
  private normalizeLisTestOrderProfiles(profiles: OpenMrsLisTestOrderProfileSeed[]): OpenMrsLisTestOrderProfileSeed[] {
    const rows: OpenMrsLisTestOrderProfileSeed[] = [];
    for (const profile of profiles ?? []) {
      ...
      if (!code || !name || analyzerCodes.length === 0) continue;

      const requiredAnalyzerCodes = this.aliasValues(null, profile?.requiredAnalyzerCodes ?? []).filter((codeValue) =>
        analyzerCodes.map((item) => item.toUpperCase()).includes(String(codeValue).toUpperCase()),
      );
