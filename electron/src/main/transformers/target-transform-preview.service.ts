...
import { getDb } from '../db/db';
import { buildOpenMrsLisPayload } from './openmrs-lis-payload.builder';
import {
    NormalizedResultRecord,
    TargetRecord,
    TransformPreviewResult,
...
export class TargetTransformPreviewService {
        ...
        return {
            ...
            lis: rawData.lis
                ? {
                      method: rawData.lis.method ?? 'POST',
                      endpoint:
                          rawData.lis.endpoint ?? '/openmrs/ws/rest/v1/lab/multipleresults',
                      sampleUuid: rawData.lis.sampleUuid ?? null,
                      sampleLabel: rawData.lis.sampleLabel ?? result.sample_id ?? null,
                      orderUuid: rawData.lis.orderUuid ?? null,
                      orderNumber: rawData.lis.orderNumber ?? result.order_id ?? null,
    ...
    private isLisBuilderMappingRule(rule: MappingRuleRow) {
        const destination = String(rule.destination_field ?? '').trim();
        return (
            destination.startsWith('lis.') ||
    ...
    private defaultPayloadForTarget(
        target: TargetRecord,
        sourceDocument: Record<string, any>,
        renderedPayload: Record<string, any>,
    ): { payload: Record<string, any>; used: boolean; warnings: string[]; errors: string[] } {
        if (target.type === 'LIS') {
            if (
                sourceDocument?.lis &&
                Array.isArray(sourceDocument.lis.body) &&
                ...
                return {
                    ...
                    payload: {
                        ...
                        method: sourceDocument.lis.method ?? 'POST',
                        endpoint:
                            sourceDocument.lis.endpoint ?? '/openmrs/ws/rest/v1/lab/multipleresults',
                        sample: {
            ...
            const generated = buildOpenMrsLisPayload(target, sourceDocument);
    ...
    private selectPreviewPayload(
        target: TargetRecord,
        renderedPayload: Record<string, any>,
        defaultPayload: { payload: Record<string, any>; used: boolean },
    ) {
        const hasRenderedPayload = Object.keys(renderedPayload).length > 0;
        if (!hasRenderedPayload) return defaultPayload.payload;
    ...
    private hasDeliverableLisPayload(payload: any) {
        if (!payload || typeof payload !== 'object') return false;
        if (Array.isArray(payload)) return true;
        if (Array.isArray(payload.body)) return true;
        if (payload.resourceType === 'OpenMRSLabMultipleResultsRequest' && Array.isArray(payload.body)) {
            return true;
        }
        if (payload.payload && Array.isArray(payload.payload.body)) return true;
        return false;
    }
