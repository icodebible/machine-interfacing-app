...
export type MappingUnmappedBehavior =
  | "PASSTHROUGH"
  | "DEFAULT_VALUE"
  | "EMPTY"
  | "ERROR";
...
export type AppAPI = {
  ...
  machinesParsedList: (
    machineId: string,
    limit?: number,
  ) => Promise<ParsedMessageRow[]>;
  machinesNormalizedList: (
    machineId: string,
    limit?: number,
  ) => Promise<NormalizedResultRow[]>;

  approvalPoliciesList: () => Promise<ApprovalPolicy[]>;
  approvalPoliciesCreate: (
    dto: Partial<ApprovalPolicy>,
  ) => Promise<{ id: string }>;
  ...
  deliveryHistoryList: (limit?: number) => Promise<OutboundQueueRow[]>;
  deliveryAuditList: (
    query?: DeliveryAuditQuery,
  ) => Promise<DeliveryAuditRow[]>;
  deliveryAuditQuery: (
    query?: DeliveryAuditQuery,
  ) => Promise<DeliveryAuditRow[]>;
  ...
  mappingsOpenMrsLisSeed: (dto: any) => Promise<any>;
  mappingValueTranslationsList: (
    mappingRuleId: string,
  ) => Promise<MappingValueTranslation[]>;
  mappingValueTranslationsCreate: (
    dto: Partial<MappingValueTranslation>,
  ) => Promise<{ id: string }>;
  mappingValueTranslationsUpdate: (
  ...
  targetTransformPreviewFromQueue: (
    queueId: string,
  ) => Promise<TargetTransformPreview>;
  targetTransformPreviewFromDeliveryHistory: (
    queueId: string,
  ) => Promise<TargetTransformPreview>;
