...
export const api: AppAPI = {
    ...
    machinesSimUseCasesList: (machineId) =>
        ipcRenderer.invoke(IPC_CHANNELS.MACHINES_SIM_USE_CASES_LIST, machineId),

    machinesSimUseCaseSave: (dto) =>
        ipcRenderer.invoke(IPC_CHANNELS.MACHINES_SIM_USE_CASE_SAVE, dto),

    machinesSimUseCaseDelete: (id) =>
        ipcRenderer.invoke(IPC_CHANNELS.MACHINES_SIM_USE_CASE_DELETE, id),

    machinesSimUseCaseRun: (machineId, useCaseId, variables) =>
        ipcRenderer.invoke(IPC_CHANNELS.MACHINES_SIM_USE_CASE_RUN, machineId, useCaseId, variables),

    machinesSimRunHistory: (machineId, limit) =>
        ipcRenderer.invoke(IPC_CHANNELS.MACHINES_SIM_RUN_HISTORY, machineId, limit),
    ...
    outboundQueueRebuildPayload: (queueId: string) =>
        ipcRenderer.invoke(IPC_CHANNELS.OUTBOUND_QUEUE_REBUILD_PAYLOAD, queueId),
    ...
    auditEventsQuery: (q: any = {}) => ipcRenderer.invoke(IPC_CHANNELS.AUDIT_EVENTS_QUERY, q),
    auditEventsSummary: (days?: number) => ipcRenderer.invoke(IPC_CHANNELS.AUDIT_EVENTS_SUMMARY, days),
    deploymentReadinessCheck: () => ipcRenderer.invoke(IPC_CHANNELS.DEPLOYMENT_READINESS_CHECK),
    appDiagnosticsGet: () => ipcRenderer.invoke(IPC_CHANNELS.APP_DIAGNOSTICS_GET),
    databaseBackupCreate: () => ipcRenderer.invoke(IPC_CHANNELS.APP_DATABASE_BACKUP_CREATE),
    ...
    lisTestOrderProfilesList: (targetId?: string | null) =>
        ipcRenderer.invoke(IPC_CHANNELS.LIS_TEST_ORDER_PROFILES_LIST, targetId ?? null),
    lisTestOrderProfileGet: (id: string) =>
        ipcRenderer.invoke(IPC_CHANNELS.LIS_TEST_ORDER_PROFILES_GET, id),
    lisTestOrderProfileSave: (payload: any) =>
        ipcRenderer.invoke(IPC_CHANNELS.LIS_TEST_ORDER_PROFILES_SAVE, payload),
    lisTestOrderProfileSetEnabled: (id: string, enabled: number) =>
        ipcRenderer.invoke(IPC_CHANNELS.LIS_TEST_ORDER_PROFILES_SET_ENABLED, id, enabled),
    lisTestOrderProfileDelete: (id: string) =>
        ipcRenderer.invoke(IPC_CHANNELS.LIS_TEST_ORDER_PROFILES_DELETE, id),
    ...
    routingRulesUpdate: (id: string, patch: any) =>
        ipcRenderer.invoke(IPC_CHANNELS.ROUTING_RULES_UPDATE, id, patch),
    routingRulesDelete: (id: string) => ipcRenderer.invoke(IPC_CHANNELS.ROUTING_RULES_DELETE, id),
